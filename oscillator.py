...
class Oscillator:
    ...
    def solve(self, t_grid: npt.NDArray[np.float64], init_cond: dict[str, float], method="euler") -> npt.NDArray[np.float64]:
        y0 = init_cond.get("y0", 0.0)
        y1 = init_cond.get("v0", 0.0)
        return self.discretize(method=method, y0=y0, y1=y1, t_grid=t_grid)
